    ...
    return (
        <Link to={`/list/${champion.id}`} className="relative flex flex-col items-center rounded-t-lg bg-slate-700/50 hover:bg-slate-600/60 transition-all group">
            <div className="relative overflow-hidden rounded-t-lg">
                <img src={champion.skins[0].splash} className='h-56 sm:h-64 lg:h-96 w-full object-cover transition-all duration-300 group-hover:brightness-50 group-hover:scale-110' alt={champion.name} />
            ...
            <div className="h-14 w-full flex items-center p-4 bg-slate-950/80">
                <h2 className="text-base sm:text-lg lg:text-xl font-bold italic truncate">{champion.name.toUpperCase()}</h2>
            </div>
        </Link>
    );
};
