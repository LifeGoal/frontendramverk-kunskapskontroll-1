...
function AdminChampionList({ champions: initialChampions }) {
    ...
    return (
        <>
            {notification && (
                <div className={`flex flex-row items-center top-4 right-4 px-4 py-2 rounded ${notification.type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white shadow`}>
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8">
                {items.sort((a, b) => a.name.localeCompare(b.name, "sv")).map(champion => (
                    <div key={champion.id} className="flex flex-row items-center gap-4 bg-slate-800 rounded-xl py-2 px-4 border border-slate-700">
                        <img src={champion.image} alt={champion.name} loading="lazy" className="w-14 h-14 rounded-full" />
                        <h1 className="text-xl italic font-bold truncate">{champion.name.toUpperCase()}</h1>
